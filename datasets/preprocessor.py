...
def build_from_path(hparams, input_dirs, mel_dir, linear_dir, wav_dir, n_jobs=12, tqdm=lambda x: x):
	...
	for input_dir in input_dirs:
		with open(os.path.join(input_dir, # enter path from your local development'metadata.csv')
				       , encoding='utf-8') as f:
			for line in f:
				parts = line.strip().split('|')
				basename = parts[0]
				wav_path = os.path.join(input_dir,# enter path from your local development'wavs',
							'{}.wav'.format(basename))
				text = parts[2]
